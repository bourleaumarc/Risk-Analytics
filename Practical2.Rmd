...
```
According to the LRT between the fixed GEV and the GEV with time varying locations (location, shape and size), we can reject the null hypothesis. This indicates that the more complex model provides a significantly better fit to the data. Furthermore, we conclude that the increase of complexity (adding time varying scale and shape) is not necessary because it does not provide a better fit. 

In conclusion, the best model to use is the GEV with time varying location parameter. 

(F) Predict the 13-years return level, each year from 2010 to 2022.
```{r}
return.level(gev_time_varying_harmonic)
as.numeric(mod3$results$par[1]+mod3$results$par[2]*((-log(1-1/13))^(-mod3$results$par[3])-1)/mod3$results$par[3])
# RL = μ_T + σ_T / ξ * [1 - (-log(1 - 1/13))^ξ]
# 2010
as.numeric()
```


Either find a predict function with a package or do it by hand, if by hand need to compute the mu for 2010-2022 and : 1-pchisq(-2*(mod2$nllh-mod1$nllh),1); k = 13. Return level should be between 139 and 157. 
