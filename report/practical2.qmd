...
test <- cbind(venice_max, yy)
mod.test <- fevd(test$max_sea_level, test, type = "GEV", time.units = "year", shape.fun = ~ yy, location.fun =  ~ yy)

ret <- c()
for(i in 1:3){
  ret[i] <- qgev(1-1/13, location=mod.test$)
}
