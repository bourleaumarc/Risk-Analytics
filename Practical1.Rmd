...
processDataFrame <- function(df) {
  ...
  df <- df[, !names(df) %in% c('DATES')]
  return(df)
}
processDataFrame(log_returns_sp500)


log_returns_sp500$date <- as.Date(rownames(log_returns_sp500))
rownames(log_returns_sp500) <- NULL
log_returns_sp500 %>% select(-2)
head(log_returns_sp500)
...
- variance is not constant: there are clear upward/downward trends. Ex: Nasdaq has very high spikes. 
