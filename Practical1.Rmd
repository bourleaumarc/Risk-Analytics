...
Using the negative log-returns, we have high p-values for the box-Ljung test, meaning that the model is capturing the temporal dependencies in the data. However, it's not the case for the Nikkei indice that has a p-value a bit low. It means that there might be autocorrelation in the residuals.
...
sp500D <- diff(sp500, lag = 1)  # For d = 1

head(sp500D)
head(sp500)

# Perform the ADF test
adf.test(sp500D, alternative = "stationary")
...
auto.arima(log_returns_cac40)
auto.arima(log_returns_nikkei)
auto.arima(log_returns_nasdaq)
